...
namespace Fiea::GameEngine{
	...
	*/
	template <typename T>
	Event<T>::Event(T Message, bool onPublishDelete) : EventPublisher(m_eventSubscribers, onPublishDelete) {
	...
	*/
	template <typename T>
	Event<T>::Event(const Event<T>& other) : EventPublisher(other), m_Message(other.m_Message) {};
	...
	*/
	...
	Event<EventType>& Event<EventType>::operator=(const Event& rhs)
	{
		if (this != &rhs) {
	...
	*/
	template<typename EventType>
	inline Event<EventType>& Event<EventType>::operator=(Event<EventType>&& rhs)
	{
		if (this != &rhs) {
